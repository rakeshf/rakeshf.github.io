...
You can run the update on GitHub using a scheduled workflow. A sample workflow has been added at `.github/workflows/update-data.yml`. It runs every 15 minutes on working days (Monday–Friday) by default and can also be triggered manually via `workflow_dispatch`.
